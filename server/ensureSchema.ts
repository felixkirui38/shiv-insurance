...
/**
 * Idempotent schema bootstrap for Coolify / production.
 * Coolify starts with `node dist/index.js` (not `yarn start`), so we cannot
 * rely on `drizzle-kit push` being available or executed at deploy time.
 * CREATE TABLE IF NOT EXISTS never drops or overwrites existing data.
 */
const SCHEMA_SQL = `
...
export async function ensureCmsSchema(): Promise<void> {
  ...
  console.log(
    "[startup] Database schema verified (users, contacts, cms_pages, cms_blog_posts, cms_testimonials, cms_downloads, cms_settings)",
  );
}
